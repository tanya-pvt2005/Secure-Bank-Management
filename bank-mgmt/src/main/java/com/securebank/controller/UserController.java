package com.securebank.controller;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;

import com.securebank.model.User;
import com.securebank.service.UserService;

@Controller

public class UserController {
	
	@Autowired
	private UserService userService = new UserService();
	
	 public void createUser(int id, String fName, String lName, String email) {
//	        User user = new User(id, fName, lName, email);
//	        userService.saveUser(user);
		 userService.saveUser(new com.securebank.model.User(id, fName, lName, email));
		 System.out.println();
	}
	 public void deleteUser(int id) {
		 userService.deleteUser(id);
		 System.out.println();
	 }
	
	 public void updateUser(int id, String fName, String lName, String email) {
	       
	        userService.updateUser(id, fName, lName, email);
	        System.out.println();
	}
	 
	 public void viewAllUsers() {
	       
	       
//	        List<User> users = userService.getAllUsers();
//	        if (users == null || users.isEmpty()) {
//	            System.out.println("No users found.");
//	            return;
//	        }
//
//	        System.out.println("=== List of Users ===");
//	        for (User user : users) {
//	            System.out.println("ID: " + user.getId() + " | " + user);
//	        }
//		 System.out.println();
//	        userService.getAllUsers().forEach(System.out::println);

		 List<User> users = userService.getAllUsers();

		 if (users == null || users.isEmpty()) {
		     System.out.println("No users found.");
		     return;
		 }

		 // Print table header
		 System.out.printf("%-5s | %-15s | %-15s | %-25s%n", "ID", "First Name", "Last Name", "Email");
		 System.out.println("---------------------------------------------------------------");

		 // Print user details
		 for (User user : users) {
		     System.out.printf("%-5d | %-15s | %-15s | %-25s%n",
		                       user.getId(),
		                       user.getFName(),
		                       user.getLName(),
		                       user.getEmail());
		 }
		 System.out.println();
	}

		
		 public void getUserById(int id) {
			    User user = userService.getUserById(id);
			    if (user != null) {
			        System.out.println("ID: " + user.getId() + ", Name: " + user.getFName() + " " + user.getLName() + ", Email: " + user.getEmail());
			    } else {
			        System.out.println("User not found with ID: " + id);
			    }
			}


	 
}
